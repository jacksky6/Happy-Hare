...
#   self._transport_name  — short label used in log messages ('pn532/i2c',
#                           'pn532/spi', 'pn532/uart')
#
# super().__init__() takes the reader's config section name as its first argument
# and stores it as self._name — the identity every log line in this family carries.
#
...
class _PN532Base:
    ...
    def __init__(self, name, transceive_delay, crc_delay, debug, low_level_debug,
        ...
        try:
            if not self._probe_status_ready():
                if self._now() > self._probe_deadline:
                    if self._debug >= 3:
                        logger.info("[%s %s] probe_poll: watchdog fired in "
                                    "stage '%s'", self._name,
            ...
            if self._probe_stage == 'ack':
                if not self._probe_fetch_ack():
                    if self._debug >= 3:
                        logger.info("[%s %s] probe_poll: bad ACK",
            ...
            if self._debug >= 3:
                logger.info("[%s %s] probe_poll: tag present uid=%s",
                            self._name, self._transport_name,
                            target_info.get('uid'))
            return True
        except PN532I2CStatusError:
    ...
    def _set_current_card(self, target_info):
        ...
        if self._debug >= 4:
            logger.info(
                "[%s %s] _set_current_card: Tg=%s UID=%s "
    ...
    def _release_current_target(self, reason="manual"):
        ...
        try:
            if self._debug >= 4:
                logger.info(
                    "[%s %s] _release_current_target: Tg=0x%02X "
                    "reason=%s", self._name, self._transport_name,
                    release_tg, reason)
            ...
            if self._debug >= 4:
                logger.info("[%s %s] _release_current_target: "
                            "error: %s\n%s", self._name, self._transport_name,
    ...
    def read_target(self, timeout=None):
        ...
        if self._debug >= 4:
            logger.info("[%s %s] read_target: scanning (timeout=%.3fs)",
                        self._name, self._transport_name, timeout)
        ...
        if target_info is None:
            self._clear_current_card()
            if self._debug >= 4:
                logger.info("[%s %s] read_target: no tag",
                            self._name, self._transport_name)
            return None
        ...
        if self._debug >= 4:
            logger.info(
                "[%s %s] read_target: Tg=%d SENS_RES=0x%04X "
    ...
    def ntag_read_page(self, page):
        ...
        if not payload or payload[0] != 0x00:
            if self._debug >= 4 and payload:
                logger.info("[%s %s] ntag_read_page: page=%d "
                            "status=0x%02X",
                            self._name, self._transport_name,
                            page, payload[0])
            return None
        ...
        try:
            ...
            while len(user_data) < target_bytes:
                ...
                if extent is not None:
                    ...
                    if target_bytes > max_ndef_bytes:
                        if self._debug >= 3:
                            logger.info(
                                "[%s %s] ntag_read_ndef_user_memory: "
                                "NDEF length=%d requires %d bytes; capped at "
                                "%d bytes by max_ndef_pages",
                                self._name, self._transport_name, ndef_len,
            ...
            if self._debug >= 4 and ndef_len is not None:
                logger.info(
                    ...
                    self._name, self._transport_name, ndef_len, len(result))
            return result
        finally:
    ...
    def mifare_authenticate(self, block_addr, key, use_key_b=False):
        ...
        if not payload or payload[0] != 0x00:
            if self._debug >= 3 and payload:
                logger.info(
                    "[%s %s] mifare_authenticate: block=%d key_%s "
                    "status=0x%02X",
                    self._name, self._transport_name, block_addr,
        ...
        try:
            ...
            for sector in sectors:
                ...
                if not self.mifare_authenticate(trailer, key, use_key_b=use_key_b):
                    ...
                    if self._debug >= 3:
                        logger.info(
                            "[%s %s] mifare_read_authenticated_blocks: "
                            "sector %d auth failed — skipping",
                            self._name, self._transport_name, sector)
                    continue
    ...
    def _wake_pn532(self, attempts=3):
        ...
        for attempt in range(attempts):
            if self._debug >= 4:
                logger.info(
                    "[%s %s] _wake_pn532: attempt %d/%d — "
                    "sending GetFirmwareVersion",
                    self._name, self._transport_name, attempt + 1, attempts)
            try:
    ...
    def init(self):
        ...
        if self._debug >= 4:
            logger.info("[%s %s] init: starting wake sequence",
                        self._name, self._transport_name)

        if not self._wake_pn532():
            raise RuntimeError(
                "[%s %s] did not respond — check wiring"
                % (self._name, self._transport_name))

        if self._debug >= 4:
            logger.info("[%s %s] init: sending SAMConfiguration "
        ...
        if not self.sam_config():
            ...
            logger.info("[%s %s] init: SAMConfiguration OK",
                        self._name, self._transport_name)
    ...
    def read_tag(self, timeout=None):
        ...
        try:
            ...
            if self._debug >= 3:
                logger.info("[%s %s] read_tag: uid=%s",
...
class PN532Driver(_PN532Base):
    ...
    def _read_ack(self, timeout=1.0, poll_interval=0.005):
        ...
        while self._now() < deadline:
            ...
            if self._debug >= 4:
                logger.info("[%s pn532/i2c] _read_ack: ready=%s",
                            self._name,
            ...
            if status == 0x01:
                try:
                    ...
                    if self._debug >= 4:
                        logger.info("[%s pn532/i2c] _read_ack: raw=%s ok=%s",
                                    self._name,
        ...
        if self._debug >= 4:
            logger.info("[%s pn532/i2c] _read_ack: timeout after %.3fs",
                        self._name, timeout)
        return False
    ...
    def _recv(self, expected_cmd_resp, read_len=_MAX_RESPONSE_BYTES,
        ...
        while self._now() < deadline:
            ...
            if self._debug >= 4:
                logger.info("[%s pn532/i2c] _recv: poll result=%s pn_status=0x%02X",
                            self._name,
                ...
                try:
                    ...
                    if self._debug >= 4:
                        ...
                        if payload is not None:
                            logger.info(
                                "[%s pn532/i2c] _recv: DATA: expect=0x%02X "
                                "pn_status=0x%02X raw=%s",
                                self._name, expected_cmd_resp, status_byte,
                                ' '.join('%02X' % b for b in raw))
                            logger.info("[%s pn532/i2c] _recv: payload: %s",
                                        self._name,
                                        ' '.join('%02X' % b for b in payload))
                        else:
                            logger.info(
                                "[%s pn532/i2c] _recv: DATA ERROR: expect=0x%02X "
                                "pn_status=0x%02X raw=%s",
                                self._name, expected_cmd_resp, status_byte,
        ...
        if self._debug >= 4:
            logger.info("[%s pn532/i2c] _recv: timeout after %.3fs waiting for ready",
                        self._name, timeout)
        return None
...
class PN532SPIDriver(_PN532Base):
    ...
    def _send(self, cmd_and_params):
        ...
        if self._debug >= 4:
            logger.info("[%s pn532/spi] _send: TX  cmd=0x%02X  frame=%s",
                        self._name, cmd_and_params[0],
                        ' '.join('%02X' % b for b in frame))
    ...
    def _read_ack(self, timeout=1.0, poll_interval=0.005):
        ...
        while self._now() < deadline:
            ...
            if self._debug >= 4:
                logger.info("[%s pn532/spi] _read_ack: status=0x%02X",
                            self._name, status)
            ...
            if status == 0x01:
                try:
                    ...
                    ok = ack == PN532_ACK
                    if self._debug >= 4:
                        logger.info("[%s pn532/spi] _read_ack: raw=%s ok=%s",
                                    self._name,
                                    ' '.join('%02X' % b for b in raw),
                                    ok)
                    return ok
        ...
        if self._debug >= 4:
            logger.info("[%s pn532/spi] _read_ack: timeout after %.3fs",
                        self._name, timeout)
        return False
    ...
    def _recv(self, expected_cmd_resp, read_len=_MAX_RESPONSE_BYTES,
        ...
        while self._now() < deadline:
            ...
            if self._debug >= 4:
                logger.info("[%s pn532/spi] _recv: poll status=0x%02X",
                            self._name, status)
            ...
            if status == 0x01:
                try:
                    ...
                    if self._debug >= 4:
                        if payload is not None:
                            logger.info(
                                "[%s pn532/spi] _recv: DATA: expect=0x%02X raw=%s",
                                self._name, expected_cmd_resp,
                                ' '.join('%02X' % b for b in raw))
                            logger.info("[%s pn532/spi] _recv: payload: %s",
                                        self._name,
                                        ' '.join('%02X' % b for b in payload))
                        else:
                            logger.info(
                                "[%s pn532/spi] _recv: DATA ERROR: expect=0x%02X raw=%s",
                                self._name, expected_cmd_resp,
                                ' '.join('%02X' % b for b in raw) if raw else '(empty)')
                    return payload
                except Exception as e:
        ...
        if self._debug >= 4:
            logger.info("[%s pn532/spi] _recv: timeout after %.3fs",
                        self._name, timeout)
        return None
